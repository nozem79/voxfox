...
# Logo lookup order: the per-user copy in PIPER_DIR (written during voice
# setup) takes priority, but fall back to the system copy shipped in the .deb
# so the About dialog always has an icon — even on a fresh install before any
# voices are downloaded.
_USER_LOGO   = os.path.join(PIPER_DIR, "voxfox-logo.png")
_SYSTEM_LOGO = "/usr/share/voxfox/voxfox-logo.png"
LOGO_PATH = _USER_LOGO if os.path.exists(_USER_LOGO) else _SYSTEM_LOGO
